...
- document.querySelector(): 
    Akzeptiert CSS-Selektoren (z. B. #id, .class, tag) und gibt das erste passende Element zurück (oder null wenn keines gefunden). Es ist flexibler und kann komplexe Selektoren verwenden, aber langsamer bei häufiger Nutzung.
- document.getElementById(): 
    Sucht spezifisch nach einem Element mit der gegebenen ID (ohne #) und gibt es direkt zurück (oder null). Es ist schneller und einfacher für ID-basierte Selektionen, aber nur für IDs geeignet.

Beide geben ein einzelnes Element zurück, aber querySelector ist vielseitiger. Für #product-list sind beide austauschbar, aber querySelector wurde verwendet, um Konsistenz mit CSS-Selektoren zu zeigen.
*/
...
// (a) Event-Listener registrieren und sortieren
const thpreis = document.getElementById("thpreis");
thpreis.addEventListener("click", () => {
    produkte.sort((a, b) => a.preis - b.preis); // Aufsteigend sortieren
    renderTable(); // Tabelle neu rendern
});

// (b) Funktion zum Rendern der Tabelle
function renderTable() {
    const tbody = document.querySelector("#tbody");
    tbody.innerHTML = ""; // Inhalt löschen
...
// Initiales Rendern (optional, aber sinnvoll)
renderTable();
```
c:
     Ohne Vergleichsfunktion konvertiert sort() Elemente zu Strings und sortiert lexikografisch (alphabetisch). Bei Zahlen (z. B. Preisen) funktioniert das nicht korrekt, da "12.90" vor "34.50" kommt, aber numerisch anders. Es führt zu falscher Reihenfolge (z. B. 12.90, 149.00, 34.50, ...).

d: Zwei Methoden:
    document.getElementById("thpreis") (schnell, nur für IDs).
    document.querySelector("#thpreis") (flexibel, verwendet CSS-Selektoren).
...
```javascript
// (a) positive: Array mit 6 Objekten (alle Studenten außer David und Hans, da Note 5 > 4)
const positive = studenten.filter((student) => student.note <= 4);

// (b) namenMitAlter: Array mit 8 Strings: ["Anna (22)", "Bernd (19)", "Clara (25)", "David (20)", "Eva (23)", "Felix (21)", "Greta (24)", "Hans (18)"]
const namenMitAlter = studenten.map((student) => `${student.name} (${student.alter})`);

// (c) durchschnitt: Zahl, ca. 2.75 (Summe 22 / 8 Studenten)
const durchschnitt = studenten.reduce((acc, student) => acc + student.note, 0) / studenten.length;

// (d) ergebnis: String "Anna,Clara,Eva,Greta" (Namen der 4 Studenten mit Alter >= 21, kommagetrennt)
...
const jsonStr = JSON.stringify(Produkt);                // Variable: produkt

// 2. Simulierter Empfang auf der Gegenseite
const empfangen = jsonStr.parse();                      // json.parse()

// 3. Preis aktualisieren
const empfangen.preis = 79.90;

// 4. Ausgabe mit Template String
console.log('Artikel: ${empfangen.name} – Preis: €${empfangen.preis}');         // `...` statt '...'

// 5. Prüfen ob auf Lager
if (empfangen.aufLager == "true") {
    console.log("Produkt ist verfügbar!");                                      // true ist hier unnötig
}
```
...
c: Erklärung zum Vergleich:
    Der Vergleich empfangen.aufLager == "true" ist problematisch, weil JSON.parse() Booleans korrekt als boolean (true/false) wiederherstellt. Nach JSON.parse() hat empfangen.aufLager den Wert true (Typ: boolean), nicht "true" (String). Mit == (loose equality) würde es funktionieren (true == "true" ist true wegen Typumwandlung), aber es ist unsicher und fehleranfällig – besser === oder direkter Boolean-Check wie if (empfangen.aufLager). == kann unerwartete Konvertierungen verursachen, z. B. 0 == false.
...
// (a) Funktion zum Rendern der Ergebnisse
function renderResults(liste) {
    const resultsDiv = document.querySelector("#results");
    resultsDiv.innerHTML = ""; // Inhalt löschen
...
// (b) Funktion zum Suchen und Filtern
